...
- (void)initBottomMenuAndEmotionView {
    ...
    NSArray *plugIcons = @[@"sharemore_pic", @"sharemore_video" ,@"watch"];
    NSArray *plugTitle = @[@"照片", @"拍摄", @"商品"];
...
- (XHMessage *)getXHMessageByMsg:(AVIMTypedMessage *)msg {
    ...
    if (msg.mediaType == kAVIMMessageMediaTypeText) {
        ...
        DLog(@"%@", imageMsg);
        xhMessage = [[XHMessage alloc] initWithPhoto:nil thumbnailUrl:imageMsg.file.url originPhotoUrl:imageMsg.file.url sender:fromUser.username timestamp:time text:[CDEmotionUtils emojiStringFromString:imageMsg.text]];
    }
    else {
...
//            disPlayViewController = messageDisplayTextView; SHOW_SINGLEPRODUCT_FROM_MESSAGE
            [NOTIFICATION_CENTER postNotificationName:SHOW_SINGLEPRODUCT_FROM_MESSAGE object:self userInfo:nil];
            break;
        }
...
- (void)sendFileMsgWithPath:(NSString *)path type:(AVIMMessageMediaType)type {
    ...
    if (type == kAVIMMessageMediaTypeImage) {
        msg = [AVIMImageMessage messageWithText:@"" attachedFilePath:path attributes:nil];
    }
    else {
...
- (void)sendImage:(UIImage *)image imageTitle:(NSString *)title{
    NSData *imageData = UIImageJPEGRepresentation(image, 0.6);
    NSString *path = [_im tmpPath];
    NSError *error;
    [imageData writeToFile:path options:NSDataWritingAtomic error:&error];
    if (error == nil) {
        [self sendFileMsgWithPath:path type:kAVIMMessageMediaTypeImage withImageTitle:title];
    }
    else {
        [self alert:@"write image to file error"];
    }
...
- (void)sendFileMsgWithPath:(NSString *)path type:(AVIMMessageMediaType)type withImageTitle:(NSString *)title{
    AVIMTypedMessage *msg;
    if (type == kAVIMMessageMediaTypeImage) {
        msg = [AVIMImageMessage messageWithText:title attachedFilePath:path attributes:nil];
    }
    else {
        msg = [AVIMAudioMessage messageWithText:nil attachedFilePath:path attributes:nil];
    }
...
//发送图片和文本消息的回调方法
- (void)didSendPhoto:(UIImage *)photo sendText:(NSString *)text fromSender:(NSString *)sender onDate:(NSDate *)date {
    [self sendImage:photo imageTitle:text];
    [self finishSendMessageWithBubbleMessageType:XHBubbleMessageMediaTypePhoto];//XHBubbleMessageMediaTypePhotoText
}
