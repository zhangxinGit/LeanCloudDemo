...
@property (nonatomic, weak, readwrite) UILabel *imageTitleLabel;
...
- (void)configureBubbleImageView:(id <XHMessageModel>)message {
    ...
    switch (currentType) {
        ...
        case XHBubbleMessageMediaTypePhoto:{
//            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"图文" message:@"photoText" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
//            [alert show];
        }
        case XHBubbleMessageMediaTypeVideo:
        case XHBubbleMessageMediaTypeLocalPosition: {
    ...
    switch (message.messageMediaType) {
        ...
        case XHBubbleMessageMediaTypePhoto:
            [_bubblePhotoImageView configureMessagePhoto:message.photo thumbnailUrl:message.thumbnailUrl originPhotoUrl:message.originPhotoUrl onBubbleMessageType:self.message.bubbleMessageType];
             _imageTitleLabel.frame = CGRectMake(0, 115, 125, 60);
             _imageTitleLabel.text = [message text];
            //message.text;
            break;
        case XHBubbleMessageMediaTypeVideo:
