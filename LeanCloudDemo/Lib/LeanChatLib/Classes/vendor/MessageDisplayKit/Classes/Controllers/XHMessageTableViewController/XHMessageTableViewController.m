...
//        case XHBubbleMessageMediaTypePhotoText: {
////            [self.messageInputView.inputTextView setText:nil];
////            if ([[[UIDevice currentDevice] systemVersion] floatValue] >= 7.0) {
////                self.messageInputView.inputTextView.enablesReturnKeyAutomatically = NO;
////                dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
...
- (void)didSendMessageWithPhoto:(UIImage *)photo withText:(NSString *)text{
    DLog(@"send photo and text : %@ %@", photo, text);
    if ([self.delegate respondsToSelector:@selector(didSendPhoto:sendText:fromSender:onDate:)]) {
        [self.delegate didSendPhoto:photo sendText:text fromSender:self.messageSender onDate:[NSDate date]];
    }
}
...
- (void)didSelecteShareMenuItem:(XHShareMenuItem *)shareMenuItem atIndex:(NSInteger)index {
    ...
    switch (index) {
        ...
        case 2: {
            ...
            [NOTIFICATION_CENTER postNotificationName:LOAD_SINGLE_PRODUCT object:self userInfo:[NSDictionary dictionaryWithObject:@(71743) forKey:@"productId"]];
            [NOTIFICATION_CENTER addObserver:self selector:@selector(sendProductImage:) name:SEND_PRODUCT_INFO object:nil];
            NSLog(@"%@",@"商品Id");
            break;
        }
        case 3: {
...
//private
- (void)sendProductImage:(NSNotification *)notification{
    [NOTIFICATION_CENTER removeObserver:self name:SEND_PRODUCT_INFO  object:nil];
    //[self didSendMessageWithPhoto:[[notification userInfo] objectForKey:@"productImage"]];
    NSDictionary *productInfoDic = [notification userInfo];
   [self didSendMessageWithPhoto:[productInfoDic objectForKey:@"productImage"] withText:[productInfoDic objectForKey:@"productName"]];
}
